...
try {
    ...
    private Authentication getMobilePasswordAuthentication(HttpServletRequest request) {
        String token = request.getHeader(JwtProperties.HEADER_STRING)
                .replace(JwtProperties.TOKEN_PREFIX,"");

        if (token != null) {
            // parse the token and validate it
            String mobile = JWT.require(HMAC512(JwtProperties.SECRET.getBytes()))
                    .build()
                    .verify(token)
                    .getSubject();
            ...
            if (mobile != null) {
              ...
              /*  user.getRoles().stream().forEach(role ->{
                    ...
                    role.getPermissions().stream().forEach(
                             userPermissions ->{
                                 if(userPermissions.getHttpPath().equals(request.getRequestURI())){
                                test.val = false;
                    }});
                });
                if(!test.val){
