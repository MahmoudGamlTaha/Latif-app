...
public interface BlogRepository extends JpaRepository<Blog, Long> {
   ...
   // @Query(value = "SELECT * FROM Blogs b WHERE   b.active = true order by created_at desc ",
     //       countQuery = "SELECT count(*) FROM Blogs b WHERE b.active = true",
       //     nativeQuery = true)
    @Query(value= "SELECT b FROM Blog b LEFT JOIN FETCH  b.userId  WHERE b.active = true  order by created_at desc",
    		countQuery = "SELECT count(*) FROM Blog b WHERE b.active = true")
    Page<Blog> findAll(Pageable pageable);
    
    @Query(value= "SELECT b FROM Blog b LEFT JOIN FETCH  b.userId  WHERE b.active = true AND b.category = ?1  order by created_at desc ",
